...
pub const Options = struct {
    ...
    alert: ?*tls.Alert = null,
    fingerprint: tls_fingerprint.Fingerprint = tls_fingerprint.default_fingerprint,
    cipher_policy: cipher_policy.CipherPolicy = cipher_policy.default_policy,
    reality_auth_key: ?*const [32]u8 = null,

    pub const entropy_len = client_hello.entropy_len;
};
...
pub fn init(input: *Reader, output: *Writer, options: Options) InitError!Client {
    ...
    var key_share = client_hello.KeyMaterial.init(options.entropy[64..240]) catch |err| switch (err) {
        // Only possible to happen if the seed is all zeroes.
        error.IdentityElement => return error.InsufficientEntropy,
    };

    var client_hello_buf: [4096]u8 = undefined;
    const out_handshake = try client_hello.buildHandshake(&client_hello_buf, .{
        .host = host,
        .entropy = options.entropy,
        .fingerprint = options.fingerprint,
        .cipher_policy = options.cipher_policy,
        .session_id = &legacy_session_id,
    }, &key_share);

    var client_hello_record_buf: [4096]u8 = undefined;
    const cleartext_header = try client_hello.buildRecord(&client_hello_record_buf, out_handshake);
    {
                ...
                switch (handshake_type) {
                    ...
                    .certificate => cert: {
                        ...
                        while (!certs_decoder.eof()) {
                            ...
                            if (cert_index == 0) {
                                if (options.reality_auth_key) |auth_key| {
                                    try verifyRealityCertificate(auth_key, subject);
...
fn verifyRealityCertificate(auth_key: *const [32]u8, subject: Certificate.Parsed) error{TlsCertificateNotVerified}!void {
    if (subject.pub_key_algo != .curveEd25519) return error.TlsCertificateNotVerified;
    const pub_key = subject.pubKey();
    if (pub_key.len != crypto.sign.Ed25519.PublicKey.encoded_length) return error.TlsCertificateNotVerified;

    var expected: [crypto.auth.hmac.sha2.HmacSha512.mac_length]u8 = undefined;
    crypto.auth.hmac.sha2.HmacSha512.create(&expected, pub_key, auth_key);
    const signature = subject.signature();
    if (signature.len != expected.len) return error.TlsCertificateNotVerified;
    if (!std.crypto.timing_safe.eql([expected.len]u8, expected, signature[0..expected.len].*)) {
